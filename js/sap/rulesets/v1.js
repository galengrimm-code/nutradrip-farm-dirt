...
window.SapRulesets = window.SapRulesets || {};

window.SapRulesets.v1 = {
  version: 'v1',
  name: 'Default Ruleset',
  description: 'Initial threshold values for sap analysis - tune based on field observations',
  ...
  thresholds: {
    corn: {
      ...
      old_leaf: {
        // Generally older leaves have lower values due to nutrient mobilization
  ...
  // Positive delta = nutrient accumulating in new leaves
  // Negative delta = nutrient mobilizing from old to new
  deltaThresholds: {
    // Nutrients that should be higher in new leaves (mobile)
    mobile: {
      Nitrogen_NO3: { concern_low: -50, concern_high: 100 }, // Should be moderate, high delta = stress
    ...
    // Nutrients typically similar or higher in old leaves (less mobile)
    immobile: {
      Calcium: { concern_low: -50, concern_high: 30 }, // Ca is immobile, old should be higher
  ...
  getThreshold(crop, tissue, nutrient) {
    const cropData = this.thresholds[crop?.toLowerCase()] || this.thresholds.corn;
    const tissueData = cropData[tissue] || cropData.new_leaf;
    return tissueData[nutrient] || null;
  },

  // Get ratio threshold
  getRatioThreshold(ratioName) {
    return this.ratios[ratioName] || null;
  }
};
